...
class Player: 
    def __init__(self, name: str, money: float = 10.0, last_bet: float = 0.0,
                 total_bet: float = 0.0, beginning_money: float = 0.0):
        self.name = name
        self.money = money
        self.hand = []
    ...
    def all_in(self):
        self.is_all_in = True
        self.is_folded = True  # locks them out of betting actions
...
class Game: 
    ...
    def betting_sequence(self, player: Player):
        ...
        while True:
            try:
                prompt_msg = (f'{player.name}: {player.show_hand()}, money: {player.money}, '
                              f'pot: {self.pot}, current bet: {self.current_bet}, '
                              f'{additional_amount} more to call -> ')
    ...
    def initial_betting_sequence(self):
        ...
        for player in self.players[2:]:
            while True:
                try:
                    msg = (f'{player.name}: {player.show_hand()}, money: {player.money}, '
                           f'pot: {self.pot}, current bet: {self.current_bet}, '
        ...
        if (self.current_bet == self.players[1].last_bet == self.big_blind):
            ...
            while True:
                try:
                    msg = (f'{self.players[1].name}: {self.players[1].show_hand()}, money: {self.players[1].money}, '
                           f'pot: {self.pot}, current bet: {self.current_bet}, 0 to check -> ')
