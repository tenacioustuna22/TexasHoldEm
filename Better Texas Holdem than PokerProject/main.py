...
class Game: 
    ...
    def bet(self, player: Player, amount: float):
        while True:
            if player.is_folded:
                return
            elif amount == 0 and player.last_bet == self.current_bet:
                player.check()
                break
    ...
    def initial_betting_sequence(self):
        for player in self.players[2:]:  
            if not player.is_folded:  
                bet_amount = float(input(f'{player.name}: {player.show_hand()}, pot: {self.pot}, current bet: {self.current_bet}, how much would you like to bet? '))
